...
export const generateSpriteSheet = async (config: SpriteSheetConfig): Promise<string> => {
  ...
  const { characterDescription, rows, style, facing, referenceImage } = config;
  ...
  // If there is a reference image, add it first
  if (referenceImage) {
    // referenceImage is a data URL like "data:image/png;base64,..."
    // We need to extract the base64 data and mimeType
  ...
  const referenceInstruction = referenceImage 
    ? "IMPORTANT: I have attached a reference image of the character. Isolate the character from this image, COMPLETELY REMOVING and IGNORING any background from the reference. The generated sprites MUST match this character's design exactly." 
    : "";
